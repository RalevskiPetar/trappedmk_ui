...
const getTotal = (items, storeReducer, clothesReducer) => {
    let total = 0
...
const Checkout = ({
    ...
    const handleCreateOrder = e => {
        ...
        form.append("cart", JSON.stringify(items.map(i => i.id)))
        setSubmitted(true)
            ...
            <div className="flex flex-col lg:h-screen h-auto lg:px-8 md:px-7 px-4 lg:py-20 md:py-10 py-6 justify-between overflow-y-auto">
                ...
                <div>
                    <div className="flex items-center pb-6 justify-between lg:pt-5 pt-20">
                        ...
                        <p className="text-2xl font-bold leading-normal text-right text-gray-800">{getTotal(items, storeReducer, clothesReducer) + 100} МКД</p>
                    </div>
                    {submitted === false ?
                        <button onClick={e => handleCreateOrder(e)} className="text-base leading-none w-full py-5 bg-gray-800 border-gray-800 border focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 text-white">Нарачај</button> :
                        cartReducer.createOrderStatus === "pending" ? null :
                            cartReducer.createOrderStatus === "error" ? "Error occurred. Please try again" :
                                cartReducer.createOrderStatus === "success" ? <Fragment>
